...
function generateSiritoriCandidates(level) {
  ...
  for (var i=from; i<level; i++) {
    for (var arr of Object.entries(siritoriList[level])) {
...
function getIdioms() {
  // var siritori = siritoriList[level];
  var siritori = generateSiritoriCandidates(level);
  ...
  while(generating) {
    for (var i=0; i<30; i++) {
      ...
      if (tmpIdioms) {
        var dupricated = true;
        for (var i=0; i<5; i++) {
          var arr = tmpIdioms[getRandomInt(0, tmpIdioms.length)].split('|');
          var idiom = arr[0];
          aiueo = arr[1];
...
// not support IE11
async function main() {
  var urls = ['/kanji-siritori/2.json', '/kanji-siritori/3.json'];
  Promise.all(urls.map(url =>
    fetch(url).then(response => response.json())
  )).then(function(results) {
    results.slice(1).forEach(result => {
      for (var i=0; i<result.length; i++) {
        for (var [aiueo, tmpIdioms] of Object.entries(result[i])) {
          var tmpSiritori = results[0][i];
