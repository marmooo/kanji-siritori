...
return false;}
function getRoute(x,y,direction,n){const directions=shuffle([1,2,3,4]);let tmpRoutes=getNeighborRoutes(x,y);if(!isPassableNeighbor(x,y,tmpRoutes)){return false;}
for(let d=0;d<directions.length;d++){if(directions[d]==1&&0<=x-n&&direction!=2){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x-i,y);if(!isPassableRoute(x-i,y,tmpRoutes)){passable=false;break;}}
if(passable)return[x-1,y,1];}
if(directions[d]==2&&x+n<size&&direction!=1){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x+i,y);if(!isPassableRoute(x+i,y,tmpRoutes)){passable=false;break;}}
if(passable)return[x+1,y,2];}
if(directions[d]==3&&0<=y-n&&direction!=4){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x,y-i);if(!isPassableRoute(x,y-i,tmpRoutes)){passable=false;break;}}
if(passable)return[x,y-1,3];}
if(directions[d]==4&&y+n<size&&direction!=3){let passable=true;for(let i=1;i<=n;i++){tmpRoutes=getNeighborRoutes(x,y+i);if(!isPassableRoute(x,y+i,tmpRoutes)){passable=false;break;}}
...
str+="\n";}
console.log(str);}
function generateGame(){let generating=true;while(generating){let x1=0;let y1=getRandomInt(1,size-1);let painting=true;counter=0;for(let x=0;x<size;x++){meiro[x]=new Array(size);for(let y=0;y<size;y++){meiro[x][y]=0;}}
...
function resizeFontSize(node){const meiroSize=document.getElementById("meiroOuter").offsetWidth;const margin=1.2;const padding=remSize*5;const border=11;const fontSize=(meiroSize-padding-border)/12/margin;node.style.fontSize=fontSize+"px";}
...
function generateSiritoriCandidates(level){const candidates={};let from=level-1;if(from<0)from=0;for(let i=from;i<level;i++){for(const[aiueo,idioms]of Object.entries(siritoriList[level])){if(candidates[aiueo]){const newIdioms=candidates[aiueo].concat(idioms);candidates[aiueo]=newIdioms;}else{candidates[aiueo]=idioms;}}}
return candidates;}
function getIdioms(){const siritori=generateSiritoriCandidates(level);const aiueos=Object.keys(siritori);let aiueo=aiueos[getRandomInt(0,aiueos.length)];let list=[];let generating=true;while(generating){for(let i=0;i<30;i++){const tmpIdioms=siritori[aiueo];if(tmpIdioms){let dupricated=true;for(let i=0;i<5;i++){const arr=tmpIdioms[getRandomInt(0,tmpIdioms.length)].split("|");const idiom=arr[0];aiueo=arr[1];if(!list.includes(idiom)){list.push(idiom);dupricated=false;break;}}
if(dupricated){break;}}else{break;}}
if(list.length==30){generating=false;}else{aiueo=aiueos[getRandomInt(0,aiueos.length)];list=[];}}
return list;}
function _initGame(){const urls=["/kanji-siritori/2.json","/kanji-siritori/3.json"];Promise.all(urls.map((url)=>fetch(url).then((response)=>response.json()))).then(function(results){results.slice(1).forEach((result)=>{for(let i=0;i<result.length;i++){const tmpSiritori=results[0][i];for(const[aiueo,tmpIdioms]of Object.entries(result[i])){if(tmpSiritori[aiueo]){tmpSiritori[aiueo]=tmpSiritori[aiueo].concat(tmpIdioms);}else{tmpSiritori[aiueo]=tmpIdioms;}}}
...
showAnswer();});});}
function _initGame2(){fetch("/kanji-siritori/2.json").then((response)=>response.json()).then((data)=>{siritoriList=data;idioms=getIdioms();generateGame();while(solvedPanel.firstChild){solvedPanel.removeChild(solvedPanel.firstChild);}
showAnswer();});}
const meiroObj=document.getElementById("meiro");resizeFontSize(meiroObj);window.addEventListener("resize",function(){resizeFontSize(meiroObj);});document.getElementById("toggleDarkMode").onclick=toggleDarkMode;document.getElementById("startButton").onclick=startGame;document.getElementById("answerButton").onclick=showAnswer;document.getElementById("hintButton").onclick=showHint;document.getElementById("levelOption").addEventListener("change",function(){level=this.selectedIndex;idioms=getIdioms();words=wordsList[this.selectedIndex];startGame();});document.getElementById("courseOption").addEventListener("change",function(){location.href=this.options[this.selectedIndex].value;});