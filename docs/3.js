...
return list;}
var timerText=document.getElementById('meiro');resizeFontSize(timerText);window.addEventListener('resize',function(){resizeFontSize(timerText);});fetch('/kanji-siritori/3.json').then(response=>response.json()).then(data=>{siritoriList=data;idioms=getIdioms();generateGame();while(solvedPanel.firstChild){solvedPanel.removeChild(solvedPanel.firstChild);}
showAnswer();});document.getElementById('levelOption').addEventListener('change',function(){level=this.selectedIndex;idioms=getIdioms();words=wordsList[this.selectedIndex];startGame();});document.getElementById('courseOption').addEventListener('change',function(){location.href=this.options[this.selectedIndex].value;});